...
export async function getGrandPrix(grandprix_iri) {
    const query = `
        ...
        ORDER BY DESC(?circuit_abstract) DESC(?circuit_thumbnail) DESC(?gp_abstract)
    `
    return requestSPARQL(query)
}
...
export async function getNumberOfGPByCircuit() {
    const query = `
        ...
        SELECT
            ?gp_name (COUNT(*) as ?nbr_gp) 
            (SAMPLE(?_latitude) as ?latitude)
            (SAMPLE(?_longitude) as ?longitude)
        WHERE {
            ?gp_uri a dbo:GrandPrix ; 
            :name ?gp_name ;
            :circuit ?circuit_uri ;
